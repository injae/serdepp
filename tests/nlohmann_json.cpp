...
TEST_CASE("2: nlohmann json variant(pass)", "[multi-file:2]") {
    auto json_v = variant_test_data();
    REQUIRE(json_v.dump() == serialize<nlohmann::json>(deserialize<std::vector<variant_test>>(json_v)).dump());
}
